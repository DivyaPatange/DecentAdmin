...
@section('page_title', 'Subject Teacher')
@section('breadcrumb', 'Subject Teacher')
@section('customcss')
                        ...
                        <div class="col-md-4">
                            <div class="form-group form-default">
                                ...
                                <select class="js-example-basic-single col-sm-12" name="section_name" id="section_name">
                                    ...
                                    ?>
                                    <option value="{{ $s->id }}">@if(!empty($class)) {{ $class->class_name }} @endif {{ $s->section_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
...
<script>
var SITEURL = '{{ route('admin.subject-teacher.index')}}';
...
$('body').on('click', '#submitForm', function () {
    ...
    { 
        var datastring="subject_name="+subject_name+"&section_name="+section_name+"&teacher_name="+teacher_name;
        // alert(datastring);
        $.ajax({
            type:"POST",
            url:"{{ route('admin.subject-teacher.store') }}",
            data:datastring,
            cache:false,        
...
$('body').on('click', '#delete', function () {
    ...
    if(confirm("Are You sure want to delete !")){
        $.ajax({
            type: "delete",
            url: "{{ url('admin/subject-teacher') }}"+'/'+id,
...
$('body').on('click', '.switch_1', function () {
    ...
    if(id != ''){
        $.ajax({
            type: "get",
            url: "{{ url('admin/subject-teacher/status') }}"+'/'+id,
            success: function (data) {
                // alert(data.teacher);
            var oTable = $('#simpletable').dataTable(); 
            oTable.fnDraw(false);
